    ...
    const { value: formValues } = await Swal.fire({
        ...
        html:  `<label style = "display:flex">
                ...
                <label style = "display:flex">
                    ...
                    <input placeholder="Teacher Photo" type="file" id="swal-input4" class="form-control swal2-input" style="width:80%"/>
                </label>
                <label style = "display:flex">
                ...
                <label style = "display:flex">
                    <p style="width:25%; margin-top: revert;">Courses</p> 
                    <select onkeyup="validation(this,4)" id="swal-input6" class="form-control swal2-input" style="width:80%">
                        <option selected>Teacher Courses</option>
                        <option>Java</option>
                        <option >C++</option>
...
function Display() {
    ...
    for(let i = 0; i < teacherRecord.length; i++) {
        ...
        val +=  `<tr>
                    ...
                    <td><img onclick="img_onclick(${i})" style="width: 110px; height: 100px; margin-top: -30px;" class="car-img" src="imgs/teacher-image/${teacherRecord[i].Photo}"></td>
                    <td>${teacherRecord[i].Position}</td>
                    <td>${teacherRecord[i].Email}</td>
                    <td>${teacherRecord[i].Address}</td>
                    <td>${teacherRecord[i].Course}</td>
                    <td style="width:10%;"><button onclick="edit_item(${i})" class="btn btn-info">Edit</button></td>
                    <td style="width:10%;"><button onclick="delete_item(${i})" class="btn btn-danger">Delete</button></td>
                </tr>`;
    }
...
function edit_item(index) {
    ...
    Course = teacherRecord[index].Course;
...
async function sweetupdate(INDEX) {
    ...
    const { value: formValues } = await Swal.fire({
        ...
        title: 'Update ' + teacherRecord[INDEX].Name + ' Reacord',
        confirmButtonText: 'Update Reacord',
        confirmButtonColor: "#4361ee",
        html:  `<label style = "display:flex"> <p style="width:25%; margin-top: revert;">Name</p>     <input placeholder="Teacher Name" onkeyup="validation(this,0)"  type="text" id="swal-input1" class="form-control swal2-input" style="width:80%"/>  </label>
                <label style = "display:flex"> <p style="width:25%; margin-top: revert;">Position</p> <input placeholder="Teacher Position" onkeyup="validation(this,1)" type="text" id="swal-input2" class="form-control swal2-input" style="width:80%"/> </label>
                <label style = "display:flex"> <p style="width:25%; margin-top: revert;">Email</p>    <input placeholder="Teacher Email" onkeyup="validation(this,2)" type="email" id="swal-input3" class="form-control swal2-input" style="width:80%"/>   </label>
                <label style = "display:flex"> <p style="width:25%; margin-top: revert;">Photo</p>    <input placeholder="Teacher Photo" type="file" id="swal-input4" class="form-control swal2-input" style="width:80%"/>                                 </label>
                <label style = "display:flex"> <p style="width:25%; margin-top: revert;">Address</p>  <input placeholder="Teacher Address" onkeyup="validation(this,3)" type="text" id="swal-input5" class="form-control swal2-input" style="width:80%"/>  </label>
                <label style = "display:flex">
                    <p style="width:25%; margin-top: revert;">Courses</p> 
                    <select onkeyup="validation(this,4)" id="swal-input6" class="form-control swal2-input" style="width:80%">
                        <option selected>Teacher Courses</option>
...
function set_data() {
    ...
    let In5 = document.getElementById('swal-input6');
...
function update(Data,index) {
    ...
    if(Photo[2] != null) {
        teacherRecord[index].Photo = Photo[2];
    }

    teacherRecord[index].Address = teacher_data[4];
    teacherRecord[index].Course = teacher_data[5];
    localStorage.setItem("teacherRecord", JSON.stringify(teacherRecord));
    Display();
}
        ...
        for(let i = 0; i < teacherRecord.length; i++) {
            ...
            if(teacherRecord[i].Name.includes(value.value)) {
                ...
                table_content +=    `<tr>
                                        <th>${i+1}</th>
                                        <td>${teacherRecord[i].Name}</td>
                                        <td><img onclick = "img_onclick(${i})" style="width: 110px; height: 100px; margin-top: -30px;" class="car-img" src="image/${teacherRecord[i].Photo}"></td>
                                        <td>${teacherRecord[i].Position}</td>
                                        <td>${teacherRecord[i].Email}</td>
                                        <td>${teacherRecord[i].Address}</td>
                                        <td>${teacherRecord[i].Course}</td>
...
function img_onclick(index) {
    ...
    Swal.fire({
        ...
        imageUrl: 'imgs/teacher-image/' + teacherRecord[index].Photo,
        imageWidth: 300,
        imageHeight: 300,
